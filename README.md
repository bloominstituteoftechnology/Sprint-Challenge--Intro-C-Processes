...
The main states are:
Running: The Process is running or ready to start.

Interruptible: A blocked state. Process and waits for a signal from another     process.

Uninterruptible: A blocked state. Process waits for a hardware condition to be met and cannot handle a signal.

Stopped: Process is stopped and can be restarted by another process.

Zombie: process terminated, information is still in the process table.


2. What is a zombie process? How does one get created? How does one get destroyed?

When a process has finished execution but still has an entry in the process table. It is created when there is nothing to cleaned it up after execution! To destroy a zombie process, destroy the parent of the zombie.

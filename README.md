...
1. List all of the main states a process may be in at any point in time on a standard Unix system. Briefly explain what each of these states means.
  ...
  Stack is the area where methods/functions, return addresses, etc are kept
  Heap is dynamically allocating memory 
  Text is the current activity 
  Data is containing the global and static variables 
2. What is a zombie process? How does one get created? How does one get destroyed?
  A zombie process is a process that has completed execution and ended with exit but still has an entry in the process table. It gets created when a child function ends but the parent has yet to be run. It can be destroyed by killing the parent process or running `kill -s SIGCHLD pid`

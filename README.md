...
Created - a process occupies this state when it's first created.
Waiting - a process occupies this state when it's been loaded and awaiting execution.
Running - a process occupies this state when it is executed by a CPU.
Blocked - a process occupies this state when it cannot continue without an external change in state.
Terminated - a process occupies this state when the running state is completed or when it's explicitly killed.

2.  What is a zombie process? How does one get created? How does one get destroyed?
    A zombie process is when a parent is terminated before the child does. Generally, a zombie process will eventually be terminated. Otherwise, a user may kill the process explicitly.

3.  What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?
    The main advantages of a compiled language are faster execution and more efficient, and they can be optimized for specific hardware.

...
Created - the process has just been created and is waiting for admission into the ready state by the scheduler.
Ready - the process has been loaded into memory and is waiting for execution.
Running - A process is chosen for execution. the instructions are executed by the CPU and run in one of two modes, Kernal mode or user mode.
Blocked - when a process cannot carry and is not yet completed.

2.  What is a zombie process? How does one get created? How does one get destroyed?
    this is a process that has completed execution but still exists in the process table. It gets destroyed once the parent process can read it's child's exit status then
    the entry if removed from the process table.

3.  What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?
    the benifits you get from working in a compiled language is that there is faster performance because you are using native code of the target machine. the benifits we gain from taking the extra time to compile our code is that we only have to run the compiler once (unless you run into errors in your code) then you can use the executable howevermany times you'd like.

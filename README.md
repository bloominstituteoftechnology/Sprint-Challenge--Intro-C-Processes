...
A Process can be in any one of the states like "new","Ready","Running" and "Terminated".
new: Process enters this state when it is first created.
...
A compiled language is one where the program, once compiled, is expressed in the instructions of the target machine.An interpreted language is one where the instructions are not directly executed by the target machine, but instead read and executed by some other program.

...
Running/ready, when the processis able to run or is running.
Waiting is when... the process is waiting for something to run.
Stopped means the process... has been stopped.

2.  What is a zombie process? How does one get created? How does one get destroyed?

A zombie process is created if it is never told to stop. So if you fork a process or something and then kill the main process but not the child process first, that process will join the army of the dead and have to be taken care of by a garbage cleaner or something, which means it'll persist for longer than it needs to.

3.  What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

Compiled languages convert more efficiently into machine code, so things can be written more easily in a specific ways to increase performance.

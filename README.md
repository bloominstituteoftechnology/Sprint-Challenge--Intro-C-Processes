...
2. What is a zombie process? How does one get created? How does one get destroyed?
  ...
  A zombie state is when a process is placed in a final state but has not yet been cleaned up.  It is created by a parent process and gets destroyed when
  zombie process sends a return code to the parent process, this then indicates to the OS that it can clean up any relevant data structures that reffered
  to the now-extint process. 

3. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

  Compiling code is only translated once by running the source code through a compiler.  This results in very efficient code that can be executed any number of times.
  The overhead for the transalation is incurred just once, when the source is compiled, thereafter, it only needs to be loaded and executed. This provides fast execution and it is usually optimized for the target hardware.

  Interpreted languages must be parsed, interpreted, and executed each time the program is run, adding to the cost of running a program.  

...
Running: The process is running its task or ready to run
Waiting/Suspended: The process is waiting some signal or  access to a resource(e.g. file)
Stopped: The process is stopped, not running and waiting. Need to restart.
Zombie: a process that is terminated but still exist according to the process table.

2. What is a zombie process? How does one get created? How does one get destroyed?
A process that is terminated but still exist according to the process table.  It can be created when a child process is finished but has not been waited on.


3. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?
The compiled code run directly on top of the operating system in assembly code versus being interpreted as it runs for non-compiled language.  So it provides faster execution and finer management of system resource.

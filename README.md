...
Created this is with the process is created as it it is new.
Waiting it is then in a state of waiting for execution.
Running the process goes into a running state when picked for execution.
Blocked the happens when the process pointer can not be found.
Terminated the process can be terminated from the running state, but it is still in the process table as a zombie.

2. What is a zombie process? How does one get created? How does one get destroyed?
A process that has finished but is still in the process table, this is created when a child runs before the parent it is a zombie while it waits for an exit from the parent.

3. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

*It runs faster because you are compiling it into code that the machine understands.
*I like that it is lower level so eveything feels very stable and defined.
*It being lower level makes everything feel a bit more controlled.

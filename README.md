...
Add your answers in-line, below, with your pull request.

1.  List all of the main states a process may be in at any point in time on a standard Unix system. Briefly explain what each of these states means.

* Running -- when a process is being run... Only one process can run on each CPU core at a time.
* Waiting -- its currently loaded into memory but not being run currently. Its waiting patiently. =D
* Blocked -- the process can not go any further, but it not yet completed. Could be waiting for user input, or trying to use a device that is not available.

2.  What is a zombie process? How does one get created? How does one get destroyed?

* A process that has completed its task, yet is still listed in the process table. Children are zombies between the time they finish and when their parent cleans them up.

3.  What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

* the compiler runs extra tests to check for typing and errors. Compiler only runs once, then you can run the executable as many times as you would like. Interpreted languages need to be build every time they are run.

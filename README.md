...
Waiting - The process is paused because it is waitinig for a signal or some information
Stopped - The process halted because it received a stop instruction

2.  What is a zombie process? How does one get created? How does one get destroyed? A zombie process is a process that is completed or no longer valid, but remains in the process table. This usually occurs when calling fork() and the child finishes before the parent. The parent process has to call wait() to clean up the zombie process and have it removed from memory.

3.  What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?
    A compiled language is less prone to having run time bugs because the program logic has been compiled before hand. Execution times are also faster than using a non-compiled language.

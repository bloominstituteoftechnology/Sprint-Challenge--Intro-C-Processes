...
1. List all of the main states a process may be in at any point in time on a standard Unix system. Briefly explain what each of these states means.
  Running: A CPU is executing a processes instructions.
  Ready: Code is prepared to run
  Blocked: A process has stopped running due to a certain condition

2. What is a zombie process? How does one get created? How does one get destroyed?
  A zombie process occurs when the child fork terminates before the parent even begins to wait on it. It could be created by giving the child a very
  simple job that will complete before the parent fork runs. The zombie process will only be destroyed when the parent process exits.

3. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?
  The main benefit of taking time to compile our code is efficiency and speed of the resulting executable. Although it may take a bit of time to initially compile the code, once it is done we have an executable that will only take a fraction of that time to run. Compiling our code also lets us hide our source code so that we can share the working program with others without having to worry about them stealing the valuable inner workings.

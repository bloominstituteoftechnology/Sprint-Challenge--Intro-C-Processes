...
* Running - In active state, or ready to be active state.
* Waiting - Waiting for an event or resource
* Stopped - non-running state
* Zombie - dead process not cleaned up

2.  What is a zombie process? How does one get created? How does one get destroyed?

* A process that has completed execution but still has an entry in the process table. Can create by removing parent, for example. Need to remove from process table.

3.  What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

* It kind of forces the developer to debug on a regular basis.

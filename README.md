...
* Running -- when a process is executing
* Waiting -- when a process is awaiting a 'green light' for execution
* Blocked -- when a process has stopped, but has more to do.

2.  What is a zombie process? How does one get created? How does one get destroyed?

* A zombie process is the event in which a process has completed but still remains in the process table. This can occur when a child process executes before a parent process. For the child process to become terminated, the parent must read the child's (now a zombie) exit status to destroy/terminate it.

3.  What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

* Non-compiled languages must compile prior to each run, slowing speed of execution, contrasting with a compiled language that is compiled once prior to running it multiple times. Taking the "extra time" (is it really "extra?") to compile our code can reduce the prevalence of run-time errors as well as bugs.

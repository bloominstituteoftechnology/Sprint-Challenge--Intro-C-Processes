...
- created: the process is created via a call to fork or exec
- ready: the process is waiting to be assigned to a central processing unit (cpu) for processing.
- running: the process is being run by a cpu
- waiting: the process is waiting for an event or resource
- terminated: after the process is finished doing what it needs to do it is ended.

2. What is a zombie process? How does one get created? How does one get destroyed?

A zombie process is a process that has been ended but still shows up as a process. The zombie process typically gets removed quickly with a wait call.

3. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

Compiled languages have to have access to all components of the program so can check things that things are getting the expected type and that things actually exist at the time of being called. By being able to check the whole program, compilers can make optimzations to the program.

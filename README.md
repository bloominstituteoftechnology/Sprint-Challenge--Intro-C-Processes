...
* Ready, Running, Waiting, Blocked, Terminated. Ready to run, Running, Waiting due to a timeout, Blocked until another process finishes, and ended.

2.  What is a zombie process? How does one get created? How does one get destroyed?

* A zombie process is a process that has completed, but is still in the stack or heap, in a terminated state because the parent process needs to read the exit status of the child (zombie) process. Once the parent reads the zombie's exit status, the zombie process is 'reaped'.

3.  What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

* compiled code is typically faster in execution. Additionally, the developer will see compilation errors, and be able to resolve them, prior to the code making it to production.

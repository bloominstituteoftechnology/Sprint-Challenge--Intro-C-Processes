...
- the main sates a process ma be in:
...
-  A zombie process is a process that has completed execution but still has an entry in the process table, it is a process in the "Terminated state". To destroy it you can send a SIGCHLD signal to the parent ```kill -s SIGCHLD <ppid>```. This will cause well-behaving parents to reap their zombie children.

3. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?
-  Compiled Languages firt compile to machine code and then execute. Non-compiled languages just execute. I thing none compile linguages are fast and thus not time consuming. 

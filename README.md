...
Zombie process is process that completed execution with exit call, but still lingering in process table.

3. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had 
from taking the extra time to compile our code?

Compiled languages can be executed directly by CPU, which makes it much faster to operate. In non-compliled languages, there's interpreter which acts as middle man between CPU and set of instructions.

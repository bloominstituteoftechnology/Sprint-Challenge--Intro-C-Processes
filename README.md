...
- Ready: A process is ready to run but for some reason the OS has chosen not to run it at this time.
- Running: In the running state, a process is running on a processor. CPU at that point in time is devoted to executing that processes instructions.
...
- a) A zombie process is when there remains an entry in the process table even after the termination of the child process.
- b) The zombie process is created when after using fork() to create a child process the parent does not wait() for the child to execute and terminate first. This means the parent can execute and terminate, therefore at the termination of the child it was left alone without a parent process and cannot be cleaned up.
- c) a zombie process can be prevented/destroyed by having the parent process call wait() which means it will wait until the child has executed/terminated first so that it can collect the states and delete the child. It works like free() in this sense.

3. What are some of the benefits of working in a compiled language versus a non-compiled language? More specifically, what benefits are there to be had from taking the extra time to compile our code?

A:

- The major advantage for complied languages is their execution speed since it is converted directly into machine code. It also gives the developer more control over things like RAM and CPU usage.
